...
> ⚠ **硬性前提**：做帧级内容对比前，必须先确认商用模块确实处于**原始 SBUS
> 透传模式**——其输出帧应为合法 SBUS 帧（严格 `0x0F` 帧头、**第 25 字节为
> `0x00`**、payload 随输入帧内容变化）。实测某次样本（`SBUS&422+&422-.csv`）
> 中商用回的是固定常量帧、帧尾非 `0x00`、内容不随输入变化，说明该次模块
> 未处于透传模式，此类样本**不能**做帧级内容对比（仅可保留"收到才发 +
> 抖动延迟"的时序结论）。重采有效数据后，用 `tools/verify-commercial.mjs`
> 复验（它会自动检查帧尾合法性与内容一致性）。
